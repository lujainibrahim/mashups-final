...
* [Link](https://docs.google.com/presentation/d/1cVTRwPkvCbbpvdiPc-1TZF9CnL5S0pLszpaqjBIrXaA/edit?usp=sharing)
### Viewing the Project:
...
As for the Financial Times API, there were additional steps that had to be carried out in order to perform an API call. For reference, the NYT API allows us to execute an API call in a single line of code, which contains the requisite configuration and parameters. In contrast, the FT API required more code because an HTTP POST method had to be implemented manually using PHP. This involved using jQuery AJAX as a mediator between the HTML-JavaScript code and the PHP code, which contains the POST instructions. Using PHP was a challenge because neither of us had previous experience with PHP, which was needed to provide the configuration and parametersof the API call as per the specifications of the FT API documentation. We overcame the gap in PHP knowledge by reading from online resources to successfully implement the FT API call.
...
<b> Feedback:</b> (quoted) - Loved the percentage graphics at the beginning, as well as the tree visualization. I also appreciated the responsiveness on the mobile version of the website.
